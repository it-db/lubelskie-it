    ...
    return (
      <motion.div className="flex overflow-hidden" variants={container} initial="hidden" animate="visible">
        {words.map((word, index) => (
          <motion.span variants={child} className={`mr-4     bg-gradient-to-r from-indigo-400 to-violet-600 bg-clip-text text-6xl font-extrabold tracking-tight text-transparent`} key={index}>
            {word}
          </motion.span>
        ))}
      </motion.div>
